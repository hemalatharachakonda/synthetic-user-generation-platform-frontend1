...
def _run_survey_via_groq(personas: list[dict], question: str, question_idx: int | None = None) -> dict | None:
    ...
    system_prompt = (
        ...
        "this persona is; don't flip a skeptic into a superfan for no reason. "
        "IMPORTANT EXCEPTION: if this question describes a feature, fix, or change "
        "that directly addresses something this persona already asked for or "
        "worried about (check prior_survey_notes/prior_interview_notes), this is "
        "NOT a reworded overall question — it's news that their specific concern "
        "was resolved, and their score should move meaningfully toward positive to "
        "reflect that (not just a token +1), while staying proportionate to how "
        "central that concern was to their hesitation. The same applies in reverse: "
        "if a question reveals their concern was NOT actually addressed, their "
        "score should move down accordingly. Every persona has a persistent memory "
        "of this whole research session: if "
